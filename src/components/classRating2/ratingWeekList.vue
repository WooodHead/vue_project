      ...
      <div class="panel" @click="onViewDetias">
        ...
        <div class="panel-middle">
          ...
          <div class="items" v-else>
            <div class="item" v-for="subItem in item.RatingDetailsList">
              <div class="item-text">{{subItem.ItemName}} {{{subItem.RatingDesc}}}</div>
  ...
  export default {
    ...
    methods: {
      loadData: function () {
        ...
        AppHelper.post(AppHelper.ApiUrls.rating2_GetClassWeekRatingList, cfg).then((jsonData) => {
          _.each(jsonData.data.ratingWeeks, function (item) {
            if (item.parent === '0') {
              item.parent = 0
            }
          })
          if (jsonData.data.sataList && jsonData.data.sataList.length > 0) {
            _.each(jsonData.data.sataList[0].RatingDetailsList, function (item) {
              if (item.RatingResultAvg > 0) {
                item.RatingDesc = item.RatingResultAvg + '☆'
              } else {
                item.RatingDesc = ''
              }
            })
            if (jsonData.data.sataList.length > 1) {
              for (var i = 1; i < jsonData.data.sataList.length; i++) {
                _.each(jsonData.data.sataList[i].RatingDetailsList, function (item) {
                  if (item.RatingResultAvg > 0) {
                    item.RatingDesc = item.RatingResultAvg + '☆'
                  } else {
                    item.RatingDesc = ''
      ...
      onViewDetias () {
        AppHelper.link('/rating2/class/list' + '?classId=' + this.classId + '&className=' + this.className)
      }
    },
    watch: {
